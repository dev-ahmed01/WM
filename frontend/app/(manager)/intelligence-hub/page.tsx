...
export default function IntelligenceHubPage() {
  ...
  useEffect(() => {
    async function fetchAnalyticsData() {
      try {
        ...
        const [usageData, confusingData, adoptionData, trendsData] = await Promise.all([
  ...
  const hasData = sopUsage.length > 0 || confusingProcedures.length > 0 || departmentAdoption.length > 0 || confidenceTrends.length > 0;
